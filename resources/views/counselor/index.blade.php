                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($results as $result)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">{{ number_format($result->age_at_exam, 1) }} yrs</td>
                                    <td class="px-6 py-4 whitespace-nowrap font-semibold">{{ $result->raw_score }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
                                            {{ $result->performance_description ?? 'N/A' }}
                                        </span>
                                        @if($result->percentile)
                                            <span class="text-xs text-gray-500">({{ $result->percentile }}%ile)</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <a href="{{ route('counselor.show', $result) }}" class="text-indigo-600 hover:text-indigo-900">
                                            Review
                                        </a>
                                    </td>
                                </tr>
